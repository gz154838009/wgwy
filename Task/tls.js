...
if ($.isNode() && process.env.TLS_tlsHEADER) {
    ...
    hyidA = process.env.TLS_hyidA || "129618";
    hyidB = process.env.TLS_hyidB || "218451";
    hyidC = process.env.TLS_hyidC || "234221";
    hyidD = process.env.TLS_hyidD || "4500279";
    hyidE = process.env.TLS_hyidE || "4250417";
    hyidF = process.env.TLS_hyidF || "4250681";
    ...
    if ("tlshyidA") {
        hyidA = $.getval("tlshyidA") || '129618';
    }
    if ("tlshyidB") {
        hyidB = $.getval("tlshyidB") || '218451';
    }
    if ("tlshyidC") {
        hyidC = $.getval("tlshyidC") || '234221';
    }
    if ("tlshyidD") {
        hyidD = $.getval("tlshyidD") || '4500279';
    }
    if ("tlshyidE") {
        hyidE = $.getval("tlshyidE") || '4250417';
    }
    if ("tlshyidF") {
        hyidF = $.getval("tlshyidF") || '4250681';
    }
    if ("tlshyidG") {
        hyidG = $.getval("tlshyidG") || '4250096';
    }
    ...
    if (!tlsheaderArr || tlsheaderArr == '') {
        ...
        for (let i = 0; i < tlsheaderArr.length; i++) {
            ...
            if (tlsheaderVal && tlsheaderVal != '') {
                ...
                HYID = [hyidA, hyidB, hyidC, hyidD, hyidE, hyidF]
                K = `好友信息🚩`;
